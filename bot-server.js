...
// ---- Custom "bying" (goodbye) messages per user — mirrors greetings.json ----
let byingsConfig = { customByings: {}, defaultBying: 'ลาก่อน' };
function loadByings() {
  try {
    byingsConfig = JSON.parse(fs.readFileSync('./byings.json', 'utf8'));
    console.log(`✅ Loaded byings.json (${Object.keys(byingsConfig.customByings || {}).length} goodbyes)`);
  } catch (err) {
    // Create a fresh file if missing so fs.watch works + saves persist.
    byingsConfig = { customByings: {}, defaultBying: 'ลาก่อน' };
    try { fs.writeFileSync('./byings.json', JSON.stringify(byingsConfig, null, 2), 'utf8'); console.log('🆕 Created byings.json'); }
    catch (e) { console.log('⚠️  Could not init byings.json:', e.message); }
  }
}
loadByings();
try {
  fs.watch('./byings.json', (eventType) => {
    if (eventType === 'change') { console.log('🔄 byings.json changed, reloading...'); loadByings(); }
  });
} catch (e) { console.log('⚠️  byings.json watch failed:', e.message); }

// ---- Save helpers (used by the regex fast-path AND the AI SET_GREETING/SET_GOODBYE commands) ----
function saveUserGreeting(uuid, name, text) {
  try {
    if (!greetingsConfig.customGreetings) greetingsConfig.customGreetings = {};
    greetingsConfig.customGreetings[uuid] = { greeting: text, name };
    fs.writeFileSync(require('path').join(__dirname, 'greetings.json'), JSON.stringify(greetingsConfig, null, 2), 'utf8');
    console.log(`✅ Saved greeting for ${name} (${uuid}): "${text}"`);
    return true;
  } catch (e) { console.error('❌ saveUserGreeting:', e.message); return false; }
}
function saveUserBying(uuid, name, text) {
  try {
    if (!byingsConfig.customByings) byingsConfig.customByings = {};
    byingsConfig.customByings[uuid] = { bying: text, name };
    fs.writeFileSync(require('path').join(__dirname, 'byings.json'), JSON.stringify(byingsConfig, null, 2), 'utf8');
    console.log(`✅ Saved bying for ${name} (${uuid}): "${text}"`);
    return true;
  } catch (e) { console.error('❌ saveUserBying:', e.message); return false; }
}
...
MUSIC: [CMD:PLAY:search query] เปิดเพลงทันที (craft good YouTube query with artist/year/genre) | [CMD:QUEUE:search query] เพิ่มเพลงในคิว/เปิดต่อ/add to queue | [CMD:SKIP] ข้ามเพลง/เพลงถัดไป/skip/next song | [CMD:PLAYLIST] ดูคิวเพลง/มีเพลงไหน/รายการเพลง/show queue | [CMD:REMOVE:N] ลบเพลงที่ N ออกจากคิว | [CMD:CLEAR_PLAYLIST] ล้างคิว/ล้างเพลงทั้งหมด | [CMD:PAUSE] หยุดเพลง/ปิดเพลง (DEFAULT for หยุด) | [CMD:RESUME] เล่นต่อ | [CMD:STOP] เลิกเล่นถาวร (ONLY for ไม่ฟังแล้ว/ปิดถาวร) | [CMD:VOLUME:N] ตั้งเสียงเป็นค่า N (N=0-200, 0=mute, 100=ปกติ, 200=ดังสุด) | [CMD:VOLUME_UP] ดังขึ้น/เพิ่มเสียง | [CMD:VOLUME_DOWN] เบาลง/ลดเสียง | [CMD:NOW_PLAYING] เพลงอะไร/เสียงเท่าไหร่
SOUND: [CMD:SOUND_HQ] ทำให้คุณภาพเสียงเพลงดีขึ้น/ชัดขึ้น/เพราะขึ้น (เช่น "ปรับเสียงให้ดีขึ้น") | [CMD:SOUND_NORMAL] ปรับเสียงกลับมาปกติ/เหมือนเดิม
PERSONAL: [CMD:SET_GREETING:คำทักทาย] ผู้ใช้อยากตั้ง/จำคำทักทายของตัวเอง (เช่น "เรียกฉันว่า...", "ทักฉันว่า...", หรือสื่อความหมายทำนองนี้ — ดึงเฉพาะคำทักทาย ไม่รวมคำสั่ง) | [CMD:SET_GOODBYE:คำบอกลา] ผู้ใช้อยากตั้งคำบอกลาตอนตัวเองออกจากห้อง (เช่น "บอกลาฉันว่า...", "เวลาฉันออกให้พูด...", หรือความหมายทำนองนี้)
RULES: SET_GREETING/SET_GOODBYE: เข้าใจแม้ผู้ใช้พิมพ์ผิดหรือใช้คำใกล้เคียง ให้ดึงเฉพาะข้อความคำทักทาย/บอกลาที่เขาต้องการ. "หยุดเพลง"=PAUSE not STOP. "เพิ่มเพลง/เปิดต่อ/add song"=QUEUE (adds to queue, not PLAY). "ข้ามเพลง/skip/next"=SKIP. "มีเพลงไหน/รายการเพลง/show queue"=PLAYLIST. "ลบเพลง N"=REMOVE:N. "ล้างคิว/clear queue"=CLEAR_PLAYLIST. PLAY=play NOW (interrupts current). VOLUME:N = absolute GME volume 0-200 (0=mute/ปิดเสียง, 25=default, 100=ปกติ/original, 200=ดังสุด/max) — use ONLY when the user names a specific level ("ตั้งเสียง 150"/"set volume to 80"). "เบาลง/ลดเสียง/quieter"=VOLUME_DOWN. "ดังขึ้น/เพิ่มเสียง/louder"=VOLUME_UP. "เสียงเท่าไหร่/what volume"=NOW_PLAYING.\n⚠️คำว่า "ปรับเสียง" กำกวม แยกให้ชัด 3 อย่าง: (a) มีตัวเลข 0-200 เช่น "ปรับเสียง 120"/"ตั้งเสียง 80" = VOLUME:N — ระดับความดัง | (b) "ปรับเสียงให้ดีขึ้น/ให้เพราะขึ้น/ให้ชัดขึ้น/คุณภาพเสียง/improve sound" = SOUND_HQ — คุณภาพเสียง(codec) ไม่ใช่ความดัง | (c) "ปรับเสียงกลับมาปกติ/เหมือนเดิม/normal sound" = SOUND_NORMAL. ส่วน "ดังขึ้น/เพิ่มเสียง"=VOLUME_UP, "เบาลง/ลดเสียง"=VOLUME_DOWN. SOUND_HQ/SOUND_NORMAL ห้ามสับสนกับ VOLUME (คนละเรื่อง: คุณภาพ vs ความดัง). Understand user INTENT in any language (Thai/English). Only [CMD:...] for clear action requests. Can combine: [CMD:JOIN_SLOT] [CMD:PLAY:query]
`;
...
async function leaveGMEVoiceRoom(botId, reason) {
  ...
  // Reset the room codec back to Fluency (mode 1) BEFORE leaving, so if a user had
  // asked to "improve the sound" (HighQuality), the room returns to normal for the
  // people still in it once the bot is gone.
  try { await axios.post(`${gmeUrl}/roomtype`, { type: 1 }, { timeout: 5000 }); } catch (e) {}
...
app.post('/api/music/volume', async (req, res) => {
  ...
  const gmeUrl = getGmeUrl(targetBotId);
  if (!gmeUrl) return res.status(500).json({ error: 'No GME process' });
  // Clamp to GME's real range [0,200]. NOTE: never `vol || default` — 0 is a
  // valid value (mute); the falsy-|| turned a 0% slide into full volume. A real
  // numeric 0 mutes; only a truly absent/blank value falls back to the default.
  const missing = vol === null || vol === undefined || vol === '' || !Number.isFinite(+vol);
  const v = missing ? DEFAULT_MUSIC_VOL : clampVol(+vol);
  botMusicVolume.set(targetBotId, v);   // persist so it survives the next song
  try {
    const resp = await axios.post(`${gmeUrl}/volume`, { vol: v }, { timeout: 5000 });
    res.json({ success: true, vol: v, ...resp.data });
  } catch (error) {
    res.status(500).json({ error: error.response?.data?.error || error.message });
  }
});
  ...
  try {
    ...
    if (playSuccess) {
      ...
      if (autoPlay) {
        ...
        // A new song is now playing → refresh the gapless preload of the NEXT one
        // (discard any stale preload first so it's based on THIS song). Fire-and-forget.
        autoPlay.nextUp = null;
        preloadNextAutoPlay(targetBotId);
      }
    }
...
// Find the next auto-play song (same-artist-first search + dedup). Shared by the live
// song-ended handler and the background preloader below. Returns {id,title,query}|null.
async function findNextAutoPlaySong(botId) {
  const autoPlay = getAutoPlayState(botId);
  ...
  if (lastSong && lastSong.title && lastSong.title !== 'Unknown') {
    ...
    if (artist) searchQuery = artist; // bare artist name → their catalog
  }
  if (!searchQuery) {
  ...
  if (!nextSong && searchResults.length > 0) nextSong = searchResults[Math.floor(Math.random() * searchResults.length)];
  if (!nextSong) return null;
  return { id: nextSong.id, title: nextSong.title, query: searchQuery };
}

// Preload (search + download to cache) the NEXT auto-play song while the current one
// is still playing, so the transition is gapless. Result kept in autoPlay.nextUp.
async function preloadNextAutoPlay(botId) {
  const autoPlay = getAutoPlayState(botId);
  if (!autoPlay || !autoPlay.enabled) return;
  if (autoPlay.preloading || autoPlay.nextUp) return; // already preloading / ready
  autoPlay.preloading = true;
  try {
    const song = await findNextAutoPlaySong(botId);
    if (!song || !song.id) return;
    const cachedFile = path.join(MUSIC_CACHE_DIR, `${song.id}.${MUSIC_EXT}`);
    let file = cachedFile;
    if (!fs.existsSync(cachedFile)) {
      file = await downloadYouTubeAudio(`https://www.youtube.com/watch?v=${song.id}`, null); // null botId = no chat spam
    }
    autoPlay.nextUp = { id: song.id, title: song.title, query: song.query, file };
    console.log(`⏭ [${botId}] Preloaded next auto-play: ${song.title}`);
    io.emit('music-log', { type: 'info', message: `[${botId}] Preloaded next: ${song.title}` });
  } catch (e) {
    console.log(`⏭ [${botId}] Preload failed: ${e.message}`);
  } finally {
    autoPlay.preloading = false;
  }
}
...
app.post('/api/music/song-ended', async (req, res) => {
  ...
  try {
    // Use the preloaded next song if the background preloader already fetched +
    // downloaded it while the previous song played (gapless). Otherwise search now.
    let nextSong = null, searchQuery = null;
    if (autoPlay.nextUp && autoPlay.nextUp.id) {
      nextSong = { id: autoPlay.nextUp.id, title: autoPlay.nextUp.title };
      searchQuery = autoPlay.nextUp.query || '';
      console.log(`[${timestamp}] ⏭ [${botId}] Using preloaded next: ${nextSong.title}`);
    } else {
      const found = await findNextAutoPlaySong(botId);
      if (found) { nextSong = { id: found.id, title: found.title }; searchQuery = found.query; }
    }
    autoPlay.nextUp = null; // consumed — a fresh one is preloaded right after this starts
...
// GME's SetAccompanyVolume is a native linear scale [0,200]: 100 = original
// recording volume, >100 boosts, <100 attenuates, 0 = mute. We expose that
// range directly (site + chat) instead of remapping it.
const botMusicVolume = new Map();  // botId -> current GME volume (0-200)
const DEFAULT_MUSIC_VOL = 25;      // default applied to every bot
const MAX_MUSIC_VOL = 200;         // GME accompaniment ceiling
const VOL_STEP = 40;               // louder/quieter step on the 0-200 scale
const clampVol = (v) => Math.max(0, Math.min(MAX_MUSIC_VOL, Math.round(v)));
const getBotVol = (botId) => botMusicVolume.get(botId) ?? DEFAULT_MUSIC_VOL;
const volEmojiFor = (v) => v === 0 ? '🔇' : v <= 60 ? '🔈' : v <= 120 ? '🔉' : '🔊';

async function executeBotCommand(action, param, botId, sender = '', senderUuid = '') {
  const timestamp = new Date().toLocaleString('th-TH', { timeZone: 'Asia/Bangkok' });
  const instance = botInstances.get(botId);
  ...
  switch (action) {
    case 'SET_GREETING': {
      ...
      if (!text || !senderUuid) { console.log(`[${timestamp}] ⚠️ SET_GREETING missing text/uuid`); return; }
      const ok = saveUserGreeting(senderUuid, sender, text);
      setTimeout(() => sendMessageForBot(botId, ok
        ? `บันทึกแล้วค่ะ! ต่อไปจะทักทาย ${sender} ว่า "${text}" 🎀`
        : `ขอโทษค่ะ บันทึกไม่ได้ 😢`), 1000);
      break;
    }
    case 'SET_GOODBYE': {
      const text = (param || '').trim();
      if (!text || !senderUuid) { console.log(`[${timestamp}] ⚠️ SET_GOODBYE missing text/uuid`); return; }
      const ok = saveUserBying(senderUuid, sender, text);
      setTimeout(() => sendMessageForBot(botId, ok
        ? `บันทึกแล้วค่ะ! ตอน ${sender} ออกจากห้องจะบอกลาว่า "${text}" 👋`
        : `ขอโทษค่ะ บันทึกไม่ได้ 😢`), 1000);
      break;
    }
    case 'JOIN_SLOT': {
    ...
    case 'SOUND_HQ':
    case 'SOUND_NORMAL': {
      // Change the room audio codec at the user's request (room-wide). 3=HighQuality
      // (48kHz stereo, "better"), 1=Fluency (normal). The bot resets to 1 when it leaves.
      const wantType = action === 'SOUND_HQ' ? 3 : 1;
      const gmeUrl = getGmeUrl(botId);
      if (!gmeUrl) {
        sendMessageForBot(botId, wantType === 3
          ? 'เปิดเพลงก่อนนะคะ แล้วค่อยบอกให้ปรับเสียงให้ดีขึ้นได้เลยค่ะ 🎵'
          : 'ตอนนี้เสียงปกติอยู่แล้วค่ะ 🔉');
        break;
      }
      try {
        await ensureGmeInRoom(botId, gmeUrl).catch(() => {}); // must be in the voice room to change its type
        const r = await axios.post(`${gmeUrl}/roomtype`, { type: wantType }, { timeout: 9000 });
        const ok = r.data && r.data.roomType === wantType;
        if (wantType === 3) {
          sendMessageForBot(botId, ok ? 'ปรับเสียงให้ดีขึ้นแล้วค่ะ 🎧 เสียงเพลงจะชัดขึ้นนะคะ' : 'ปรับเสียงไม่สำเร็จค่ะ ลองใหม่นะคะ 🙏');
        } else {
          sendMessageForBot(botId, ok ? 'ปรับเสียงกลับมาปกติแล้วค่ะ 🔉' : 'ปรับเสียงไม่สำเร็จค่ะ 🙏');
        }
      } catch (e) {
        sendMessageForBot(botId, 'ขอโทษค่ะ ปรับเสียงไม่ได้ตอนนี้ 🙏');
      }
      break;
    }
    case 'PLAY': {
    ...
    case 'VOLUME_UP': {
      const gmeUrl = getGmeUrl(botId);
      if (!gmeUrl) break;
      const newVol = clampVol(getBotVol(botId) + VOL_STEP);
      botMusicVolume.set(botId, newVol);
      try {
        await axios.post(`${gmeUrl}/volume`, { vol: newVol }, { timeout: 5000 });
        console.log(`[${timestamp}] 🔊 AI VOLUME_UP → ${newVol}/${MAX_MUSIC_VOL}`);
        setTimeout(() => sendMessageForBot(botId, `${volEmojiFor(newVol)} ปรับเสียงเป็น ${newVol}/${MAX_MUSIC_VOL}`), 1500);
      } catch (err) {
    ...
    case 'VOLUME_DOWN': {
      ...
      try {
        await axios.post(`${gmeUrl}/volume`, { vol: newVol }, { timeout: 5000 });
        console.log(`[${timestamp}] 🔉 AI VOLUME_DOWN → ${newVol}/${MAX_MUSIC_VOL}`);
        setTimeout(() => sendMessageForBot(botId, `${volEmojiFor(newVol)} ปรับเสียงเป็น ${newVol}/${MAX_MUSIC_VOL}`), 1500);
      } catch (err) {
    ...
    case 'VOLUME': {
      ...
      // Absolute GME volume, native 0-200 (100 = original). No remapping.
      const parsed = parseInt(param, 10);
      const gmeVol = clampVol(isNaN(parsed) ? getBotVol(botId) : parsed);
      botMusicVolume.set(botId, gmeVol);
      try {
        await axios.post(`${gmeUrl}/volume`, { vol: gmeVol }, { timeout: 5000 });
        console.log(`[${timestamp}] ${volEmojiFor(gmeVol)} AI VOLUME → ${gmeVol}/${MAX_MUSIC_VOL}`);
        setTimeout(() => {
          sendMessageForBot(botId, `${volEmojiFor(gmeVol)} ปรับเสียงเป็น ${gmeVol}/${MAX_MUSIC_VOL}`);
        }, 1500);
      } catch (err) {
    ...
    case 'NOW_PLAYING': {
      ...
      try {
        ...
        const curVol = getBotVol(botId);
        if (status.playing) {
          setTimeout(() => {
            sendMessageForBot(botId, `🎵 กำลังเล่น: ${status.currentFile || 'ไม่ทราบชื่อเพลง'}\n${volEmojiFor(curVol)} เสียง: ${curVol}/${MAX_MUSIC_VOL}`);
          }, 1500);
        } else {
          setTimeout(() => {
        ...
        console.log(`[${timestamp}] ℹ️ AI NOW_PLAYING: playing=${status.playing}, vol=${curVol}/${MAX_MUSIC_VOL}`);
      } catch (err) {
        ...
        if (!isBotMessage) {
          ...
          if (hasTrigger) {
            ...
            if (customGreeting && customGreeting.length > 0) {
              ...
              const ok = saveUserGreeting(senderUuid, sender, customGreeting);
              setTimeout(() => {
                sendMessageForBot(targetBotId, ok
                  ? `บันทึกแล้วค่ะ! ต่อไป ${botConfig.name} จะทักทาย ${sender} ว่า "${customGreeting}" 🎀`
                  : `ขอโทษค่ะ ไม่สามารถบันทึกได้ 😢`);
              }, 1000);
              return; // Don't process as AI question
            }

            // Check if user wants to set their custom GOODBYE (bying) — regex fast-path.
            // Patterns: "บอกลาฉันว่า X", "เวลาฉันออกให้พูด X", "say bye to me X"
            const byingPatterns = [
              /(?:ช่วย)?บอกลา(?:ฉัน|ผม|เรา|หนู)(?:ว่า|ด้วย)\s*(.+)/i,
              /(?:ตอน|เวลา)(?:ฉัน|ผม|เรา|หนู)?ออก(?:ห้อง|จากห้อง|ไป)?\s*(?:ให้)?(?:พูด|บอก)(?:ว่า)?\s*(.+)/i,
              /say\s*(?:good)?bye\s*(?:to\s*me)?\s*(?:with|as)?\s+(.+)/i,
              /set\s*(?:my)?\s*(?:goodbye|farewell|bye)\s*(?:to|as)?\s+(.+)/i,
            ];
            let customBying = null;
            for (const pattern of byingPatterns) {
              const m = question.match(pattern);
              if (m && m[1]) { customBying = m[1].trim(); break; }
            }
            if (customBying && customBying.length > 0) {
              console.log(`[${timestamp}] 👋 ${sender} (${senderUuid}) wants to set custom goodbye: "${customBying}"`);
              const ok = saveUserBying(senderUuid, sender, customBying);
              setTimeout(() => {
                sendMessageForBot(targetBotId, ok
                  ? `บันทึกแล้วค่ะ! ตอน ${sender} ออกจากห้อง ${botConfig.name} จะบอกลาว่า "${customBying}" 👋`
                  : `ขอโทษค่ะ ไม่สามารถบันทึกได้ 😢`);
              }, 1000);
              return; // Don't process as AI question
            }
            ...
            getAIResponse(question, senderUuid, sender, botConfig.name, targetBotId)
              .then(async (aiReply) => {
                ...
                if (commands.length > 0) {
                  ...
                  for (const cmd of commands) {
                    try {
                      await executeBotCommand(cmd.action, cmd.param, targetBotId, sender, senderUuid);
                    } catch (cmdErr) {
        ...
        instance.previousParticipants.forEach((prevName, prevUuid) => {
          if (!prevName?.includes(botConfig.name) && !currentParticipants.has(prevUuid)) {
            ...
            if (joinInfo) {
              ...
              // Custom per-user goodbye (byings.json) if set, else the default farewell.
              const uuidBying = byingsConfig.customByings?.[prevUuid];
              const byingText = uuidBying ? (typeof uuidBying === 'object' ? uuidBying.bying : uuidBying) : null;
              const goodbye = byingText ? `${byingText} ${userName}` : `ลาก่อน ${userName} (อยู่ ${timeStr})`;
...
function initOperator() {
  ...
  operator = createOperator({
    ...
    isBlockedUser: (name) => BLOCKED_USERNAMES.some((b) => (name || '').toLowerCase().includes(b)),
